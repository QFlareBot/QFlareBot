...
同时打开面板（用自定义域名），概览页的「最近事件」里应该能看到这条消息：平时从 Workers Logs 读，大约晚 15 秒；想立刻看到、连消息原文一起看，先点「开始实时调试」再发。收不到的话见[配置 QQ 开放平台 · 常见问题](./deploy-qq#常见问题)。

## 接下来

- 自带的几个插件能做什么：[内置插件](./builtin-plugins)
- 装更多插件：[插件市场](./market)，或在面板「插件」页粘贴仓库链接
- 以后怎么升级、构建失败了看哪里：[升级与运维](./maintenance)
